    ...
    try {
        ...
        if (command.kind === "observe-console") {
            ...
            const handler = (msg) => {
                ...
                if (command.options.level && command.options.level !== level)
                    return;
        ...
        if (command.kind === "observe-requests") {
            ...
            const handler = (req) => {
                const url = req.url?.() ?? "";
                if (command.options.filter && !url.includes(command.options.filter))
                    return;
        ...
        if (command.kind === "snapshot") {
            const snapshot = await page.evaluate(({ maxText, maxItems, includeHtml }) => {
            ...
            if (command.includeA11y) {
                snapshot.a11y = await page.accessibility.snapshot();
            }
        ...
        if (command.kind === "click") {
            ...
            try {
                ...
                const withFrames = async (runner) => runInChildFrames(page, runner);
                const clickWithLocator = async (timeoutMs) => {
        ...
        if (command.kind === "type") {
            try {
                const timeout = command.options.timeoutMs;
                const delayMs = command.options.delayMs !== undefined ? Math.max(0, command.options.delayMs) : undefined;
                const plainText = isLikelyPlainText(command.selector);
                const typeViaLocator = async (target, selector) => {
                    const locator = target.locator(selector).first();
                    await locator.click({ timeout });
                    await locator.type(command.text, { timeout, delay: delayMs });
                };
                const fillViaLocator = async (target, selector) => {
                ...
                const attempts = [
                    ...
                    },
                    {
                        ...
                        run: async () => {
                            ...
                            const count = await locator.count();
                            if (count !== 1)
                                throw new Error("No unique visible textbox on page");
                    ...
                    },
                    {
                        ...
                        run: async () => {
                            await runInChildFrames(page, async (frame) => {
                                ...
                                const count = await locator.count();
                                if (count !== 1)
                                    throw new Error("No unique visible textbox in frame");
                                await locator.first().fill(command.text, { timeout });
                            });
                        }
                    }
        ...
        if (command.kind === "drag") {
            try {
                ...
                const getTarget = (scope, query, isText) => (isText ? scope.getByText(query) : scope.locator(query)).first();
                const dragInScope = async (scope) => {
        ...
        if (command.kind === "select") {
            try {
                ...
                const attempts = [];
                const clickAndChoose = async (clickTarget, scope) => {
                    await clickTarget();
                    ...
                    run: async () => {
                        await page.evaluate(({ selector, value }) => {
                            ...
                            if (!(el instanceof HTMLSelectElement))
                                throw new Error("Selector is not a <select>");
                            const option = Array.from(el.options).find((o) => o.value === value || o.label === value || o.text === value);
                            if (!option)
                                throw new Error("Option not found in select");
                ...
                const attempts = [
                    ...
                    },
                    {
                        ...
                        run: async () => {
                            ...
                            const count = await locator.count();
                            if (count !== 1)
                                throw new Error("No unique file input on page");
                    ...
                    },
                    {
                        ...
                        run: async () => {
                            await runInChildFrames(page, async (frame) => {
                                ...
                                const count = await locator.count();
                                if (count !== 1)
                                    throw new Error("No unique file input in frame");
                                await locator.first().setInputFiles(command.files, { timeout });
                            });
                        }
                    }
        ...
        if (command.kind === "wait") {
            const timeout = command.options.waitTimeoutMs !== undefined ? command.options.waitTimeoutMs : command.options.timeoutMs;
            try {
                ...
                if (command.options.text) {
                    await page.waitForFunction((t) => document.body && document.body.innerText && document.body.innerText.includes(String(t)), command.options.text, { timeout });
                }
...
function emit(options, envelope, legacyText) {
    if (options.json) {
...
async function runInChildFrames(page, runner) {
    const frames = page.frames().filter((f) => f !== page.mainFrame());
    if (frames.length === 0) {
...
async function mouseDragBetween(page, source, target, timeout) {
...
async function safeUserAgent(page) {
    try {
...
async function actionError(error, browser, options, details) {
    ...
    const hint = `Selected tab: ${details.tabIndex}\n` +
