export function isLikelyCssSelector(selector) {
...
export function isLikelyPlainText(selector) {
    const trimmed = selector.trim();
    if (!trimmed)
        return false;
    const lower = trimmed.toLowerCase();
...
export async function resolveSmartSelector(page, originalSelector, options) {
    ...
    try {
        ...
        if (debug) {
            console.log(`Original selector "${originalSelector}" failed: ${e.message}`);
        }
    }
    ...
    for (let level = 1; level <= maxTraversals; level++) {
        try {
            ...
            if (!parentSelector) {
                if (debug)
                    console.log(`No parent found at level ${level}`);
                continue;
            }
            ...
            if (debug) {
                console.log(`Parent selector at level ${level} failed: ${e.message}`);
            }
        }
    }
    ...
    if (options.tryCss !== false) {
        try {
            ...
            if (debug)
                console.log(`CSS selector generation failed: ${e.message}`);
        }
    }
    ...
    if (options.tryText !== false) {
        try {
            ...
            if (debug)
                console.log(`Text selector generation failed: ${e.message}`);
        }
    }
...
async function generateParentSelector(page, selector, level) {
...
async function generateCssSelector(page, originalSelector) {
    return page.evaluate((sel) => {
        const el = document.querySelector(sel);
        if (!el)
            return null;
...
async function generateTextSelector(page, originalSelector) {
    return page.evaluate((sel) => {
        const el = document.querySelector(sel);
        if (!el)
            return null;
        ...
        if (text.length < 50 && text.length > 0) {
            ...
            const matches = document.evaluate(`//*[contains(text(), "${text.replace(/"/g, '\\"')}")]`, document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
...
export function formatSelectorResult(result) {
    if (!result.success) {
