export type Command =
  | {
      kind: "navigate";
      url: string;
      options: GlobalOptions & { waitUntil: WaitUntil };
    }
  | {
      kind: "eval-js";
      code: string;
  ...
  | {
      ...
      selector: string;
      text: string;
      options: GlobalOptions & { delayMs?: number };
    }
  | {
      kind: "press";
      key: string;
  ...
  | {
      kind: "highlight";
      selector: string;
      options: GlobalOptions & { durationMs: number };
    }
  | {
      kind: "resize";
      width: number;
  ...
  | {
      ...
      options: GlobalOptions & { timeMs: number; level?: string };
    }
  | {
      kind: "observe-requests";
      options: GlobalOptions & { timeMs: number; filter?: string };
    }
...
const DEFAULTS: Omit<
  GlobalOptions,
  "tabIndex" | "tabUrlContains" | "chromeExecutable" | "userDataDir" | "chromeProfileDir" | "pwProfile" | "display"
> = {
...
export function parseArgv(argv: string[]): Command {
  ...
  if (command === "navigate") {
    ...
    if (!url) return { kind: "help", text: helpText() };
  ...
  if (command === "eval-js") {
    ...
    if (!code) return { kind: "help", text: helpText() };
  ...
  if (command === "profiles") {
    const sub = positionals[0];
    if (sub === "list") return { kind: "profiles-list", options: optionsToGlobals(options) };
    if (sub === "path") {
      const name = positionals[1];
      if (!name) return { kind: "help", text: helpText() };
  ...
  if (command === "click") {
    ...
    if (!selector) return { kind: "help", text: helpText() };
    const button: "left" | "right" | "middle" = options.has("--right") ? "right" : options.has("--middle") ? "middle" : "left";
  ...
  if (command === "type") {
    ...
    if (!selector || text === undefined) return { kind: "help", text: helpText() };
  ...
  if (command === "press") {
    ...
    if (!key) return { kind: "help", text: helpText() };
    const delayRaw = getString(options, "--delay-ms");
    const delayMs = delayRaw ? Number(delayRaw) : undefined;
  ...
  if (command === "drag") {
    ...
    if (!startSelector || !endSelector) return { kind: "help", text: helpText() };
  ...
  if (command === "fill") {
    ...
    if (!selector || value === undefined) return { kind: "help", text: helpText() };
  ...
  if (command === "fill-label") {
    ...
    const value = positionals[1];
    if (!label || value === undefined) return { kind: "help", text: helpText() };
  ...
  if (command === "fill-role") {
    ...
    if (!role || !name || value === undefined) return { kind: "help", text: helpText() };
  ...
  if (command === "select") {
    ...
    if (!selector || values.length === 0) return { kind: "help", text: helpText() };
  ...
  if (command === "upload") {
    ...
    if (!selector || files.length === 0) return { kind: "help", text: helpText() };
  ...
  if (command === "wait") {
    ...
    const waitUrlContains =
      waitUrlRaw ?? (options.has("--tab-url-contains") ? undefined : getString(options, "--url-contains"));
  ...
  if (command === "resize") {
    ...
    if (!Number.isFinite(width) || !Number.isFinite(height)) return { kind: "help", text: helpText() };
...
function parseWaitUntil(raw: string | undefined): WaitUntil {
  ...
  return waitUntilRaw === "domcontentloaded" || waitUntilRaw === "networkidle" || waitUntilRaw === "load"
    ? (waitUntilRaw as WaitUntil)
    : "load";
}

function optionsToGlobals(map: Map<string, string | true>): GlobalOptions {
  ...
  const cdpUrl = String(
    getString(map, "--cdp-url") ?? `http://127.0.0.1:${Number.isFinite(cdpPort) ? cdpPort : 9222}`
  );
...
function parseOptions(tokens: string[]) {
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    if (requiresValue.has(token)) {
      if (!hasValue) throw new Error(`Missing value for ${token}`);
      options.set(token, next as string);
      index++;
      continue;
    }
