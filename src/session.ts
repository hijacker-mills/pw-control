...
import type { GlobalOptions } from "./cli_args.js";
...
export type PageSelectOptions = Pick<GlobalOptions, "tabIndex" | "tabUrlContains">;

export async function connectOrLaunchChrome(options: GlobalOptions): Promise<{ browser: Browser; cleanup: () => Promise<void> }> {
  ...
  if (!options.forceLaunch) {
    try {
      ...
      if (!options.launchChrome) {
        throw new Error(
          `Failed to connect to Chrome at ${cdpUrl}. Start Chrome with --remote-debugging-port=${options.cdpPort} or pass --launch-chrome.`
        );
      }
    }
  }
...
export async function ensureContext(browser: Browser): Promise<BrowserContext> {
...
export async function ensurePage(
...
): Promise<{ context: BrowserContext; page: Page; index: number }> {
  ...
  if (select?.tabIndex !== undefined) {
    ...
    if (!page) throw new Error(`Tab index out of range: ${idx}`);
...
async function spawnDetached(executable: string, args: string[], candidates: string[], env: NodeJS.ProcessEnv) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    if (anyErr?.code === "ENOENT") {
      const hint = candidates
        ...
        .join("\n");
      throw new Error(
        `Could not start Chrome/Chromium. Executable not found: ${executable}\n` +
...
async function waitForChrome(cdpUrl: string, timeoutMs: number) {
  ...
  throw new Error(
    `Timed out waiting for Chrome at ${cdpUrl}. ` +
